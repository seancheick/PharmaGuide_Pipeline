...
> Status: ACTIVE — **v2.5.0:** certification claims and verification are separate. `certification_detail.claimed_programs` lists every label claim; `certification_detail.verified_programs` lists registry-verified product certifications with `record_id`, `scope`, `source_url`, `snapshot_date`, `recency_status`. Legacy `certification_detail.third_party_programs.programs` is verified-only (`source: "registry"` + `record_id`); `purity_verified` / `heavy_metal_tested` / `label_accuracy_verified`, core `has_third_party_testing` and `cert_programs` come from verified programs only. A label claim never lights a verified surface. 2.4.0 catalogs remain importable; readers must treat a legacy program entry without `source: "registry"` as a claim. Legacy `verdict` `POOR` is the quality verdict of the lowest shipped `quality_tier` (quality_score 1.9.0); it is never a safety finding. **v2.3.0:** adds the typed `v4_dose_safety` detail-blob summary and preserves v2.2.0's independent `product_safety_status` / `quality_assessment_status` fields. Legacy `verdict` remains temporarily for compatibility, but consumer safety behavior must use `product_safety_status`; `POOR` and low-coverage `CAUTION` are not safety findings. **v2.0.0 BREAKING:** the production catalog is v4 only. The legacy /80 columns `score_quality_80` + `score_display_80` are **DROPPED**; the canonical shipped score is `quality_score_v4_100` (/100), with `score_100_equivalent` / `score_display_100_equivalent` as honest /100 compat mirrors.
>
...
CREATE TABLE products_core (
    ...
    safety_signal_reason          TEXT,                -- scored CAUTION/suppressed safety signal reason
    ...
    has_third_party_testing       INTEGER DEFAULT 0,   -- registry-verified product certification (2.5.0+)
...
| `safety_signal_reason`         | V4 safety gate signal or suppression reason               | Separate reason field for scored CAUTION rows and suppressed safety rows                 |
