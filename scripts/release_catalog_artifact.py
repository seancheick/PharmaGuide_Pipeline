...
def is_confirmed_ban_or_recall(product: Dict[str, Any]) -> bool:
    ...
    The upstream safety gate owns jurisdiction, role and policy eligibility.
    This reads its recorded decision; it does not infer a new ban from an
...
def stage_release(
    *,
    ...
    try:
        ...
        # Carry the per-product detail artifacts forward from the source
        # `final_db_output/` into the new staging dir. These are produced
        # once during catalog assembly (build_final_db.py) alongside the
        # DB — NOT regenerated by any downstream pipeline step. Without
        # this copy, the Supabase sync step fails because
        # `detail_index.json` is missing from dist/.
        #
        # Both must exist in the source: build_final_db.py always writes
        # them together with pharmaguide_core.db. A missing artifact
        # means upstream assembly was incomplete — fail loudly rather
        # than silently ship a partial release.
        source_dir = validation["db_path"].parent
        src_detail_index = source_dir / "detail_index.json"
        src_detail_blobs = source_dir / "detail_blobs"
        _require(
            src_detail_index.is_file(),
            f"Release candidate missing detail_index.json at {src_detail_index}. "
            f"build_final_db.py writes this alongside pharmaguide_core.db — "
            f"re-run catalog assembly before staging.",
        )
        _require(
            src_detail_blobs.is_dir(),
            f"Release candidate missing detail_blobs/ at {src_detail_blobs}. "
            f"build_final_db.py writes this alongside pharmaguide_core.db — "
            f"re-run catalog assembly before staging.",
        )
