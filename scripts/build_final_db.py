...
def build_detail_blob(
    ...
    *,
    ...
    for alert in safe_list(safe_dict(enriched.get("interaction_profile")).get("ingredient_alerts")):
        ...
        for hit in safe_list(alert.get("dose_hits")):
            if isinstance(hit, dict):
                raw_sev = safe_str(hit.get("severity"), "moderate").lower()
                warnings.append({
                    "type": "interaction",
                    "severity": raw_sev,
                    "severity_contextual": _INTERACTION_CONTEXTUAL_SEVERITY.get(raw_sev, raw_sev),
                    "display_mode_default": _INTERACTION_DISPLAY_MODE.get(raw_sev, "suppress"),
                    "title": f"{ing_name} / dose",
                    "detail": safe_str(hit.get("alert_body")),
                    "alert_headline": hit.get("alert_headline"),
                    "alert_body": hit.get("alert_body"),
                    "ingredient_name": ing_name,
                    "ingredient_canonical_id": ingredient_canonical_id or None,
                    "sources": safe_list(hit.get("sources")),
                    "dose_threshold_evaluation": None,
                    "dose_decision": compact_dose_decision(hit.get("dose_decision")),
                    "source": "interaction_rules",
                    "source_rule_id": safe_str(alert.get("rule_id")) or None,
                    "profile_gate": hit.get("profile_gate"),
                })
