...
| [`unit_conversions.json`](unit_conversions.json)                               | Multi-section payload; `total_entries` tracks `vitamin_conversions` only — other sub-dicts are static rule/alias config                                                                       | [test_unit_conversions_contract.py](../tests/test_unit_conversions_contract.py)                               | `total_entries == len(vitamin_conversions)` + all 4 sections present                                                                                                      |
