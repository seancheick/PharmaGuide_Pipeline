...
| **Catalog disposition** | The product-level decision that distinguishes a scoring candidate from an intentionally non-scoreable QA record. `intentional_non_scoreable` requires explicit label evidence such as external-only use, professional formulation material, culinary use, sweetener use, or a verified emergency-only policy signal that agrees with the label; it never makes a product live and carries a typed reason plus source paths. |
