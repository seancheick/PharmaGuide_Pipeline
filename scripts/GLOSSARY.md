...
| **Research scope unresolved** | `scope_unresolved` means the registry has not established whether research applies to an exact strain, a whole formula, or a broader taxon. It is not species-level evidence. Previously reviewed contextual research can remain credited without awarding dose-applicability points or an affirmative specificity badge. |
| **Evidence search coverage** | A dated, bounded record of the sources, queries and clinical questions reviewed. It is separate from research strength and product applicability; a partial search never proves evidence is absent. |
| **Native study context** | One source-backed organism/formula, population, indication, regimen and outcome record within the existing probiotic registry. Discrete trial arms are not a continuous dose window; viability measurements and one-time challenge doses are not daily efficacy thresholds. |
| **Exact identity completeness** | The fraction of distinct probiotic label identities that resolve to exact registered strains through their own source rows. Structured forms are resolved before duplicate projections and registered aliases are collapsed. Unresolved label names remain in the denominator; derived count fields and detached clinical IDs cannot supply identities. Formulation awards 8 times this fraction, independently of strain count, CFU size, clinical efficacy, or prebiotic complement. |
| **Label-dose comparability** | Whether the source-owned label dose and delivery match a reported study regimen. It is separate from clinical benefit, clinician approval and suitability for a patient. A comparable amount alone cannot award outcome-applicability points. |
| **Human clinical evidence unestablished** | Native source review does not establish human clinical evidence. Animal/laboratory sources and unresolved source type do not earn human clinical Evidence points. This is a knowledge gap, not a finding of ineffectiveness; an independent valid backbone match remains eligible. |
| **Native evidence effect direction** | The direction of the registered primary source's assessed outcome, independent of study quality. It applies only to that native credit, not to other ingredients or independent evidence sources. A null result is not missing research. |
| **Evidence credit owner** | The evidence module's attribution of the points that actually survive caps and combination math: `strain`, `companion`, `mixed`, `studied_formula`, or `none`. Presentation reads this result and never guesses ownership from ingredient names. |
| **Native research review incomplete** | Identity or source approval is still unresolved for the native evidence. Zero current credit in this state does not conclude no benefit or poor product quality. |
...
| **Label-owned strain identity** | An exact registry identity re-proved against its source label row, independently of whether the registry's clinical citation is approved. Duplicate projections of the same source are one identity; identity alone does not establish clinical benefit. |
| **Aggregate CFU disclosure** | A disclosed viable count for a whole product or blend. It is not an equal-share estimate or a measured dose of any individual strain. |
...
| **Native research provenance** | `source_pmids` inventories the reference and source-verified study contexts; `scoring_source_pmids` identifies only the existing point-producing reference. Pending contexts cannot borrow that reference's approval. `native_context_review` is independent of a positive/null/negative result and of the numerical score. |
| **Native context clinical approval** | An attributable `clinical_review` record on a `clinician_approved` native study context. It names a non-placeholder reviewer, a timezone-aware review timestamp, and the exact `identity_dose_outcome_applicability` scope. A status string without this provenance cannot authorize scoring. |
