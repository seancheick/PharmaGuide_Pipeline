...
def example_usage():
    """Example usage of FunctionalGroupingHandler"""

    handler = FunctionalGroupingHandler()

    test_cases = [
        "Natural Colors: Beet Root Powder, Turmeric Extract",
        "Natural Colors",
        "Natural Flavors",
        "Natural Flavors: Vanilla Extract, Lemon Oil",
        "Magnesium Stearate",
        "Artificial Colors",
        "Preservatives: Ascorbic Acid, Tocopherols"
    ]

    print("=" * 80)
    print("FUNCTIONAL GROUPING HANDLER - TEST CASES")
    print("=" * 80)
    print()
    ...
    for ingredient in test_cases:
        ...
        # Cleaning process
        cleaned = handler.process_ingredient_for_cleaning(ingredient)
        print(f"Type: {cleaned['type']}")
        print(f"Transparency: {cleaned['transparency']}")

        if cleaned['type'] == 'functional_group_with_details':
            print(f"Functional Type: {cleaned['functional_type']}")
            print(f"Specific Ingredients: {', '.join(cleaned['ingredients'])}")
            print(f"✅ GOOD: Specific disclosure")

        elif cleaned['type'] in ['functional_group_vague', 'vague_declaration']:
            print(f"⚠️  WARNING: Vague disclosure")
            if 'vague_flags' in cleaned:
                for flag in cleaned['vague_flags']:
                    print(f"   Flag: {flag['flag']}")
                    print(f"   Penalty: {flag['penalty']}")

        # Enrichment scoring
        scoring = handler.score_transparency_for_enrichment(cleaned)
        print(f"\nTransparency Score: {scoring['transparency_score']}")

        if scoring['bonuses']:
            print(f"Bonuses: +{sum(b['bonus'] for b in scoring['bonuses'])}")

        if scoring['penalties']:
            print(f"Penalties: {sum(p['penalty'] for p in scoring['penalties'])}")
