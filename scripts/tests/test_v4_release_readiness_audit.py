...
def test_soft_disclosure_debt_is_ok_without_cap_or_ceiling() -> None:
    row = _row(v4_completeness_soft_missing=["low_confidence_omega_breakdown"])

    assert classify_row(row) == "OK_SCORED_WITH_SOFT_DISCLOSURE_DEBT"


def test_completeness_cap_or_ceiling_is_release_blocker() -> None:
    capped = _row(
        v4_completeness_soft_missing=["low_confidence_omega_breakdown"],
        v4_completeness_score_cap=65.0,
    )

    assert classify_row(capped) == "BLOCKER_COMPLETENESS_CAP_OR_CEILING"
