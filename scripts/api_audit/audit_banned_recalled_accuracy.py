...
Primary operator commands:
  ...
  - Run a fresh FDA sync and then audit:
      .venv/bin/python scripts/api_audit/audit_banned_recalled_accuracy.py --run-fda-sync --days 30 --fda-report-out scripts/fda_sync_report_latest.json
  - Production gate:
...
def run_fda_sync(days: int, output_path: Path) -> dict[str, Any]:
    cmd = [
        sys.executable,
        str(SCRIPT_DIR / "fda_weekly_sync.py"),
        "--days",
        str(days),
        "--output",
        str(output_path),
