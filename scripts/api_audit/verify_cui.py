...
Operator runbook:
  1. Dry-run the whole file first:
       python3 scripts/api_audit/verify_cui.py --file scripts/data/banned_recalled_ingredients.json --list-key ingredients --id-field id --cui-field cui
  2. Inspect a single missing or suspicious concept:
       python3 scripts/api_audit/verify_cui.py --search "sildenafil"
       python3 scripts/api_audit/verify_cui.py --cui C0529793
  3. Apply only safe exact fills for genuinely missing CUIs:
       python3 scripts/api_audit/verify_cui.py --file scripts/data/banned_recalled_ingredients.json --list-key ingredients --id-field id --cui-field cui --apply
  4. Only use --apply-mismatches after manual review of the FDA/UMLS evidence:
       python3 scripts/api_audit/verify_cui.py --file scripts/data/banned_recalled_ingredients.json --list-key ingredients --id-field id --cui-field cui --apply --apply-mismatches
...
VERIFY_CUI_HELP_EPILOG = """Examples:
  ...
  Safe exact-match apply only:
    .venv/bin/python scripts/api_audit/verify_cui.py --file scripts/data/banned_recalled_ingredients.json --list-key ingredients --id-field id --cui-field cui --apply

  Reviewed mismatch overwrite:
    .venv/bin/python scripts/api_audit/verify_cui.py --file scripts/data/banned_recalled_ingredients.json --list-key ingredients --id-field id --cui-field cui --apply --apply-mismatches
"""
...
def _exact_match_is_safe_for_entry(
    *,
    ...
    standard_tokens = _token_set(standard_name)
    latin_tokens = _token_set(latin_name)
    matched_tokens = _token_set(matched_term)
    candidate_tokens = _token_set(candidate_name)

    if matched_tokens and matched_tokens != standard_tokens and matched_tokens != latin_tokens:
        if matched_tokens & BOTANICAL_NARROWING_TOKENS:
            return False

    extra_candidate_tokens = (candidate_tokens & BOTANICAL_NARROWING_TOKENS) - standard_tokens - latin_tokens
    return not extra_candidate_tokens
...
def verify_cui_for_entry(
    ...
    suppress_exact_null_review = has_approved_null and (entity_type or "").lower() == "product"
    ...
    if report["status"] in ("unknown", "INVALID_CUI", "MISMATCH") or not current_cui:
        ...
        if not block_further_search:
            ...
            if exact and not report["suggested_cui"] and _exact_match_is_safe_for_entry(
                standard_name=standard_name,
                candidate_name=exact["name"],
                ...
                if suppress_exact_null_review:
                    report["status"] = "ANNOTATED_NULL"
                    report["action"] = None
                elif has_approved_null:
            ...
            if latin_name and not report["suggested_cui"] and report["status"] != "NOT_FOUND":
                latin_match = client.search_exact(latin_name)
                if latin_match and _exact_match_is_safe_for_entry(
                    standard_name=standard_name,
                    candidate_name=latin_match["name"],
                    matched_term=latin_name,
                    latin_name=latin_name,
                ):
                    report["suggested_cui"] = latin_match["cui"]
                    report["suggested_name"] = latin_match["name"]
                    report["match_source"] = "exact_latin_name"
                    if suppress_exact_null_review:
                        report["status"] = "ANNOTATED_NULL"
                        report["action"] = None
                    elif has_approved_null:
                        report["status"] = "ANNOTATED_NULL_REVIEW"
                        report["action"] = (
                            f"Annotated null should be reviewed; exact latin-name match found: "
                            f"{latin_match['cui']} ({latin_match['name']})"
                        )
                    elif not current_cui:
                        report["status"] = "MISSING_CUI"
                        report["action"] = f"Suggest CUI {latin_match['cui']} ({latin_match['name']})"
            if not report["suggested_cui"] and report["status"] != "NOT_FOUND":
                for alias in aliases:
                    ...
                    if not alias_match:
                        continue
                    if not _exact_match_is_safe_for_entry(
                        standard_name=standard_name,
                        candidate_name=alias_match["name"],
                        matched_term=alias,
                        latin_name=latin_name,
                    ):
                        continue
...
def should_apply_cui_fix(report: dict, *, allow_mismatch_overwrite: bool = False) -> bool:
    ...
    if report.get("status") == "MISSING_CUI":
        return match_source in {"exact_standard", "exact_alias", "exact_latin_name", "curated_override"}
...
def load_entries(file_path: Path, list_key: str, id_field: str, cui_field: str, *, mode: str = "flat") -> list[dict]:
    ...
    if mode == "iqm":
        ...
        for key, value in data.items():
            if key.startswith("_") or not isinstance(value, dict):
                continue
            entry = dict(value)
            entry.setdefault(id_field, key)
            entry.setdefault("standard_name", value.get("standard_name") or value.get("display_name") or key)
...
def cmd_verify(args):
    ...
    entries = load_entries(file_path, args.list_key, args.id_field, args.cui_field, mode=args.mode)
        ...
        if args.mode == "iqm":
            for key, entry in data.items():
                if key.startswith("_") or not isinstance(entry, dict):
                    continue
                eid = entry.get(args.id_field, key)
...
def main():
    ...
    parser.add_argument("--mode", choices=["flat", "iqm"], default="flat",
                        help="File shape: flat top-level list or IQM top-level map (default: flat)")
    ...
    parser.add_argument("--cache-file", default=str(SCRIPTS_ROOT / ".cache" / "umls_api_cache.json"),
