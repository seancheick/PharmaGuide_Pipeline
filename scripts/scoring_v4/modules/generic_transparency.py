...
def _score_b5_proprietary_blend_penalty(
    ...
    for blend in blends:
        if not isinstance(blend, dict):
            continue
        if not _is_b5_scoreable_blend(blend):
            continue
        key = _blend_dedupe_fingerprint(blend)
        if key in seen_keys:
            continue
        seen_keys.add(key)
...
def _get_disclosure_blends(product: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if blends:
        return [b for b in blends if isinstance(b, dict)]
    return [b for b in _safe_list(_safe_dict(product.get("proprietary_data")).get("blends", [])) if isinstance(b, dict)]
...
def _blend_dedupe_fingerprint(blend: Dict[str, Any]) -> Tuple[str, Tuple[str, ...], str, str]:
    ...
    blend_total_mg, _ = _quantity_to_mg(amount_value, amount_unit)
