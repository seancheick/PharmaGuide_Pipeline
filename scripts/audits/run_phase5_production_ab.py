...
def _evaluate_single_enriched_file(args: Tuple[str, str]) -> Dict[str, Any]:
    ...
    for enriched_p in prods:
        ...
        # Rule 5/6: The not_scored products are separate; production-scored universe is evaluated.
...
def run_ab_audit() -> Dict[str, Any]:
    ...
    for i, task in enumerate(tasks):
        if (i + 1) % 10 == 0 or i + 1 == len(tasks):
            print(f"  Processed {i + 1}/{len(tasks)} brands...")
        partial_res = _evaluate_single_enriched_file(task)
