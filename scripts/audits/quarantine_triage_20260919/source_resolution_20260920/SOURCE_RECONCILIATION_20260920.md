...
| 201420 | folate residual (name) | declared-total row **named `Folic Acid`** 1333 mcg DFE + nested `Folic Acid` 800 mcg | identical | scan not legible for this row | `source_verified_correction` — **CLOSED, not by a row rewrite**: the declared-total row and its child share the identical raw text `Folic Acid`, so a rename cannot be scoped to the total alone. Implemented structurally in the folate dose-safety contract, which now identifies the declared total from structure (Daily-Value-anchored row, else the declared-total DFE basis) rather than from a parent-name vocabulary. **0 data rows changed**; folate charged once (1333 mcg DFE = 79.96 %UL); the independent niacin exceedance and `CAUTION` verdict preserved; now identical to sibling 201405. See `../FOLATE_201420_CLOSURE_20260920.md`. | No |
...
Distinct products with a correction receipt: **10** — **all 10 applied and
replay-verified** (9 source rewrites plus `201420`, closed structurally with zero
data rows changed). No receipt is left reported-but-unapplied. Basis 2 and 3 of
`COUNT_BASIS_20260920.md` carry the detail, and the measured evidence for the
closure is in `FOLATE_201420_CLOSURE_20260920.md`.

## Non-source items that remain (and are *not* source questions)

1. ~~**One reviewed dose-safety reconciliation change** (folate parent/child basis)~~ — **CLOSED 2026-09-20**: the declared total is now identified from structure (Daily-Value anchor, else the declared-total DFE basis), with negative controls for two Daily-Value-anchored rows and for differently named DFE rows. Fixtures `201420`, `243808`, `246430`, guard `228823`; measured blast radius on the full frozen corpus: **one changed product**. See `../FOLATE_201420_CLOSURE_20260920.md`.
...
6. ~~**`201420` folate double-count**~~ — **CLOSED**: one product, opened and closed as an engine fix (0 data rows changed); no engineering follow-up remains open.
