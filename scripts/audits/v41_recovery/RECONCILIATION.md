...
- 168 IQM parents have no authored unspecified form; an undisclosed form of them takes the derived, identity-neutral lowest - 1 until an entry is curated. L-tyrosine's lowest form is D-tyrosine (2), so its derived value is 1; author "l-tyrosine (unspecified)" (NALT 8 - 1 = 7).
- A row whose only form token is unresolved falls back to its name, and a bare-name alias can then supply a named form ("L-Glutamine" as glutamine alpha-ketoglutarate, which IQM lacks, reads as L-glutamine powder 11). Named-but-unresolved tokens should stay unmapped per the policy.
- 75 IQM parents score their authored unspecified form at or above their best other form (e.g. pine bark 12 vs Pycnogenol 10); audit against the unspecified policy one entry at a time. Census: `scripts/audits/v41_recovery/unspecified_census.py`.
- 159 of the 168 carry the bare parent name as an alias on a named form, so an undisclosed label ("HMB", "Leucine") still resolves to that form by exact alias (HMB -> HMB-Ca 13). 123 are single-form parents where the form is the identity (pregnenolone, L-histidine); the 36 multi-form parents need one-entry review (HMB, citrus bioflavonoids, inositol, DIM, caffeine, NAC, CLA, BCAA, krill oil, ...).
- 21 parents name their default form "standard" ("branched chain amino acids (standard)" 10, "nattokinase standard" 9); the provider does not read "standard" as authored unspecified. Decide whether it is.
