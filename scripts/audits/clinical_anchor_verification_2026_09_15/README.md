...
## Scoring effect (quality_score 1.5.0)

`generic_dose._CLINICAL_ANCHOR_REFERENCE_BY_CANONICAL` now lists only verified or corrected
anchors that appear in product adequacy rows (quercetin, betaine, glycine, GABA, berberine,
beta-alanine, glucosamine, hyaluronic acid, NMN, PQQ, colostrum, urolithin A). Those earn
credit in proportion to the anchor up to 100%; every other anchor keeps the legacy
full-credit-at-25% rule. `test_clinical_anchor_map_lists_only_pubmed_verified_anchors` ties the
map to this ledger and to the reference values.
...
glycine inside blends). Extra searches for a positive controlled trial of PQQ at 10 mg or GABA
below 100 mg found none.
