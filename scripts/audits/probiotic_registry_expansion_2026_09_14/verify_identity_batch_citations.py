...
import json, re, time, unicodedata, urllib.request, urllib.parse, sys
...
def fulltext(p):
    ...
    if pmc.get(p):
        try:
            full[p] = bound_pmc_text(get(f'https://www.ebi.ac.uk/europepmc/webservices/rest/{pmc[p]}/fullTextXML'), p)
...
for op in spec['operations']:
    ...
    for e in op['evidence']:
        checked += 1; q = e['quote']
        if source_quote_matches(q, abstract.get(e['pmid'])): continue
        if source_quote_matches(q, fulltext(e['pmid'])): via_full += 1; continue
...
raise SystemExit(1 if fails or set(pmids) - set(abstract) else 0)
