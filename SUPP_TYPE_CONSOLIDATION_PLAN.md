...
6. ~~Begin at **Phase -1**, not Phase 0a.~~ **Phases -1, 0a and 0c are COMPLETE, committed, and green as of 2026-07-16** (see §8's "PHASE -1 IS COMPLETE" note, §9's "0a AS BUILT" note, and the 0b/0c notes). The harness is trustworthy with a zero noise floor; the SoT gate is off the path literal and off prose; the §5 dependency order is enforced and behaviourally guarded. **Begin at Phase 0b** (now unblocked — percentile inference runs after the taxonomy), then 0d. Do not re-derive what those notes already measured; verify the symbols still exist, then continue.
