...
|---|---|---|
| **Big score number** | `score_100` | The actual 0–100 quality score |
...
| **P1.5 — omega module decision checkpoint** | Run the generic-module shadow scorer specifically against the omega-3 canary subset (rows 5–9). Compare expected vs actual rank order and review IFOS/EPA/DHA/oxidation signals. **Decision gate:** does generic-module handle omega-3 acceptably, or does omega need its own module before P2? Codex flagged this — fish oil signals (IFOS lot star ratings, EPA:DHA ratio, oxidation) are not the same as generic minerals. | Either (a) omega rows rank correctly within ±1 → omega stays in `generic` for v1, or (b) omega rows fail → add `omega` module to the v1 scope BEFORE P2 |
