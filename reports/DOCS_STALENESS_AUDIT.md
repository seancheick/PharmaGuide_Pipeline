...
- `docs/PHARMAGUIDE_FLUTTER_ROADMAP.md`
...
**Audit prompt templates (move to `scripts/audits/prompts/`):**
- `Harmful_additive_audit_prompt.md` (top-level)
- `Ingredient_Aliases_accuracy_audit.md` (top-level)
- `UNMAPPED_RESOLUTION_PROMPT.md` (top-level)
...
| `resume-optimizer-prompt.md` | Unrelated to pipeline (a resume tool prompt) |
| `scripts/IQM_AUDIT_MASTER_PROMPT.md` | Audit-prompt template; agents derive equivalents fresh per session |
| `scripts/PROMPT_ADD_INTERACTION_RULES.md` | Authoring prompt; SOP doc supersedes it |
...
## IGNORE_BY_AGENTS — 4 patterns (agent-side guard rails)

Add to `CLAUDE.md` / `AGENTS.md` so agents know NOT to treat these as implementation truth:

| Pattern | Why |
|---|---|
| `docs/superpowers/*` | AI-generated design specs (March / April) — not current implementation; only useful as historical "what we considered" |
| `docs/plans/*` | Old plans — superseded by Sprint E1.x and beyond |
| `.claude/skills/*` | Agent skill definitions; not pipeline docs |
| `scripts/audits/**/research.md` | Per-batch research notes; transient, deleted after each batch ships per `feedback_*` memory |
...
1. Commit the working-tree change to `docs/PIPELINE_OPERATIONS_README.md`, then either delete it or merge it into the `scripts/` canonical copy
2. Execute the DELETE list — 14 files
3. Execute the ARCHIVE list — move 46 files to `docs/archive/YYYY-MM/`
4. Apply the 5 UPDATE-bucket refreshes
5. Patch `CLAUDE.md` / `AGENTS.md` with the truth-priority paragraph
6. Optional follow-up audit script: walk the keep-list and assert every claim is verifiable in code/data
